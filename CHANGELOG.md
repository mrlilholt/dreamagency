...
- Updated Firestore mission suggestion publishing to reset per-option feedback/import fields on each regenerate so new content does not inherit prior likes/dislikes and disappear from the default untagged list (`scripts/publishMissionSuggestions.mjs`).
- Expanded feedback-driven generation controls so missions now avoid class-name openers, enforce stronger class-specific framing (CS digital/debug, ID multi-discipline systems, DREAM creative future blend), increase novelty, and add varied context lead-ins for each prompt (`scripts/syncMissionFeedback.mjs`, `scripts/generateDailyMissions.mjs`).
- Updated mission generation so every prompt begins with a scenario opener (`Scenario: You are ...`) and class-specific scenario contexts are varied for CS, DREAM, and ID tracks (`scripts/generateDailyMissions.mjs`, `.codex/automations/daily-mission-creator/automation.toml`).
- Reworked mission prompt tone to read like incoming email requests (subject, sender voice, background, stakes, and ask) so briefs feel conversational and less formulaic while still preserving sketch requirements and class focus (`scripts/generateDailyMissions.mjs`, `.codex/automations/daily-mission-creator/automation.toml`).
